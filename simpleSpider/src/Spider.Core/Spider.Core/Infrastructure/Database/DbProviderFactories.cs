...
namespace Spider.Core.Infrastructure.Database
{
    /// <summary>
        ...
        static DbProviderFactories()
        {
            ...
            foreach (var assembly in baseProviders)
            {
                ...
                if (factoryType != null)
                {
                    ...
                    if (instanceField?.GetValue(null) is DbProviderFactory)
                    {
                        DbProviderFactory instance = (DbProviderFactory)instanceField?.GetValue(null);
